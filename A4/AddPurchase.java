...
public class AddPurchase {
	...
	private String whenp = "2021-04-11 10:01:23"; // time of purchase
	private Integer qnty = 1; // number of copies purchased
	private String currentTime;
	//SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
	//currentTime = (formatter.format(new Date(System.currentTimeMillis())));
	//DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
	//LocalDateTime now = LocalDateTime.now();currentTime=(dtf.format(now)).toString();whenp=currentTime;
	...
	public AddPurchase(String[] args) {
		...
		try {
			...
			props.setProperty("user", "vaidya28");
		...
		try {
			...
			//conDB = DriverManager.getConnection(url,"vaidya28","217761016");
		...
		// Customer
		if (args[0].equals("-c") != true) { // Also check length
	...
	public boolean offerCheck() {
		...
		try {
			querySt.setString(1, club.toString());
			querySt.setString(2, title.toString());
	...
	public void insertPurchase(int cid, String club, String title, int year, String when, int qnty) {
		...
		// queryText = "SELECT title, year " + "FROM yrb_offer " + " WHERE club = ? AND
		// title = ? AND year = ? ";
		...
		try {
			...
			querySt.setString(5, when.toString());
